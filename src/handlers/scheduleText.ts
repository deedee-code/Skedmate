 ...
 */
export async function handleAwaitingContent(
  phone: string,
  msg: BufferedMessage
): Promise<void> {
 ...
 */
export async function handleAwaitingRecipient(
  phone: string,
  msg: BufferedMessage
): Promise<void> {
  let recipient = '';
 ...
 */
export async function handleAwaitingTime(
  phone: string,
  msg: BufferedMessage
): Promise<void> {
  ...
  const recurrence = ctx.recurrence as string | undefined;

  // Save to DB
  const schedule = await prisma.schedule.create({
    data: {
      userId: user!.id,
      recipient: ctx.recipient as string,
