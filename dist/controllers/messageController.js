...
import { handleAwaitingContent, handleAwaitingRecipient, handleAwaitingTime, } from '../handlers/scheduleText.js';
import { handleAwaitingBroadcastNumbers, handleAwaitingBroadcastConfirm, handleAwaitingBroadcastContent, handleAwaitingBroadcastTime, } from '../handlers/broadcast.js';
import { handleAwaitingRecurrence, handleAwaitingCustomRecurrence, } from '../handlers/recurring.js';
import { handleAwaitingCancelId } from '../handlers/cancelSchedule.js';
import { handleAwaitingSettingsChoice, handleAwaitingNewTimezone, } from '../handlers/settings.js';
/**
 * Central message router.
 * Called by the buffer after grouping text + media from the same user.
 */
export async function processMessage(phone, msg) {
    try {
        // Look up the user
        ...
        switch (state) {
            ...
            case 'AWAITING_RECIPIENT':
                if (mode === 'recurring') {
                    ...
                    await sendText(phone, 'How often should this repeat?\n\nReply:\ndaily\nweekly\nmonthly\ncustom');
                }
                else {
                    await handleAwaitingRecipient(phone, msg);
                }
        ...
        await sendText(phone, "Oops, something went wrong on my end 😅 Please try again in a moment.");
    }
}
