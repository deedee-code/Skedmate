 ...
 */
export async function handleAwaitingNewTimezone(phone, msg) {
    const input = msg.text?.trim() ?? '';
    // Validate the timezone
    if (!isValidTimezone(input)) {
        await sendText(phone, `"${input}" doesn't look like a valid timezone 🤔\n\nTry one of:\n${COMMON_TIMEZONES.join('\n')}`);
        return;
    }
